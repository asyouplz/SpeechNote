...
export interface AppEventMap extends EventMap {
    ...
    'editor:text-inserted': {
        text: string;
        position?: ObsidianEditorPosition;
...
export class EventManager extends EventEmitter<AppEventMap> {
     ...
     */
    on<K extends keyof AppEventMap>(event: K, listener: EventListener<AppEventMap[K]>): Unsubscribe;
    ...
    getStatus(): EventManagerStatus {
        ...
        return {
            ...
            totalListeners: eventNames.reduce(
                (sum: number, event) => sum + this.listenerCount(event),
                0
            ),
            totalEmitted: Array.from(this.eventStats.values()).reduce(
                (sum, count) => sum + count,
                0
            ),
