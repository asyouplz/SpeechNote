...
export class PerformanceBenchmark {
     ...
     * 메트릭 기록
     */
    static recordMetric(name: string, value: number, metadata?: Record<string, unknown>): void {
            ...
            if (threshold) {
                if (stats.mean > threshold.critical) {
                    ...
                    recommendations.push(
                        `⚠️ ${name}: Mean time ${stats.mean.toFixed(
                            2
                        )}ms exceeds critical threshold ${threshold.critical}ms`
                    );
                } else if (stats.mean > threshold.warning) {
                    recommendations.push(
                        `⚡ ${name}: Mean time ${stats.mean.toFixed(
                            2
                        )}ms exceeds warning threshold ${threshold.warning}ms`
                    );
     ...
     * 임계값 설정
     */
    static setThreshold(name: string, target: number, warning: number, critical: number): void {
    ...
    private static checkThreshold(name: string, value: number): void {
        ...
        if (value > threshold.critical) {
            console.error(
                `Performance critical: ${name} = ${value.toFixed(2)}ms (threshold: ${
                    threshold.critical
                }ms)`
            );
        } else if (value > threshold.warning) {
            console.warn(
                `Performance warning: ${name} = ${value.toFixed(2)}ms (threshold: ${
                    threshold.warning
                }ms)`
            );
        }
            ...
            try {
                const observer = new PerformanceObserver((list) => {
                    for (const entry of list.getEntries()) {
                        this.recordMetric(`${type}.${entry.name}`, entry.duration, {
    ...
    static measureNetworkTiming(): void {
        ...
        if (navigation) {
            ...
            // TCP 연결 시간
            this.recordMetric('network.tcp', navigation.connectEnd - navigation.connectStart);

            // TTFB (Time to First Byte)
            this.recordMetric('network.ttfb', navigation.responseStart - navigation.requestStart);
            ...
            // 전체 로드 시간
            this.recordMetric('network.total', navigation.loadEventEnd - navigation.fetchStart);
        }
    }
