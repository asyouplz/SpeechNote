...
import { DiarizationFormatter, DiarizedWord, DiarizationConfig } from './DiarizationFormatter';
import { DEEPGRAM_API, AUDIO_VALIDATION, ERROR_MESSAGES } from './constants';
        ...
        for (let attempt = 0; attempt < this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries - 1) {
                    ...
                    this.logger.debug(
                        `Deepgram: Retrying after ${delay}ms (attempt ${attempt + 1}/${
                            this.maxRetries
                        })`
                    );
...
export class DeepgramService {
                    ...
                    this.logger.debug('First channel analysis', {
                        ...
                        firstAlternative: firstChannel.alternatives?.[0]
                            ? {
                                  ...
                                  transcriptLength:
                                      firstChannel.alternatives[0].transcript?.length || 0,
                                  transcriptEmpty:
                                      !firstChannel.alternatives[0].transcript ||
                                      firstChannel.alternatives[0].transcript.trim() === '',
                                  transcriptPreview:
                                      firstChannel.alternatives[0].transcript?.substring(0, 200) +
                                      (firstChannel.alternatives[0].transcript &&
                                      firstChannel.alternatives[0].transcript.length > 200
                                          ? '...'
                                          : ''),
                                  ...
                                  firstFewWords: firstChannel.alternatives[0].words
                                      ?.slice(0, 5)
                                      .map((w) => ({
                                          word: w.word,
    ...
    private buildHeaders(detectedFormat?: string): Record<string, string> {
        ...
        const contentType =
            detectedFormat && contentTypeMap[detectedFormat]
    ...
    parseResponse(
        ...
        const isEmptyTranscript =
            !alternative.transcript || alternative.transcript.trim().length === 0;
        if (isEmptyTranscript) {
            ...
            if (!alternative.words || alternative.words.length === 0) {
                ...
                diagnosticInfo.push(
                    `${alternative.words.length} words detected but empty transcript`
                );
            }
            ...
            const errorDetails =
                diagnosticInfo.length > 0 ? ` Possible causes: ${diagnosticInfo.join(', ')}` : '';
        ...
        if (diarizationConfig?.enabled && hasSpeakerInfo) {
            ...
            try {
                ...
                this.logger.error(
                    'Diarization formatting failed, falling back to original transcript',
                    error as Error
                );
            ...
            if (alternative.words && alternative.words.length > 0) {
                segments = this.createSegmentsFromWords(alternative.words);
                this.logger.debug(
                    `Created ${segments.length} segments from ${alternative.words.length} words`
                );
            }
        }
